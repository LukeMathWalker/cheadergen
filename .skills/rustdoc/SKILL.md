...
**Re-exported items (same crate):** Rustdoc generates the doc page at the _re-export_ location, not the original definition site. For example, if a crate re-exports `foo::bar::Baz` as `foo::Baz`, the page lives at `struct.Baz.html` in the crate root, not under `bar/`. If a direct path based on the original module doesn't work:
