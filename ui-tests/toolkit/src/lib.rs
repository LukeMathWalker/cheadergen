...
pub use test_manifest::{
    TestManifest, TestManifestError, read_manifest_file, read_test_manifest, write_manifest_file,
};
...
pub fn normalize_stderr(stderr: &str, workspace_root: &Path) -> String {
    let root_str = workspace_root.to_str().expect("non-UTF-8 workspace root");
